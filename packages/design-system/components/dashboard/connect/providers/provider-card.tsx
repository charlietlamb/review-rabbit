...
export default function ProviderCard({ provider }: { provider: ProviderData }) {
  ...
  const iconElement = isValidElement(provider.icon)
    ? cloneElement(provider.icon as ReactElement<{ className?: string }>, {
        className: cn(
          'w-12 h-12 mb-2 border-black',
          provider.icon.props?.className
        ),
      })
    : provider.icon

  return (
    <div className="flex-grow w-full bg-black/80 rounded-lg">
      <button
        className={cn(
          'text-lg font-bold font-heading p-2 transition-all duration-300 rounded-lg flex flex-col items-center justify-center md:py-8 cursor-pointer text-white w-full',
          provider.className
        )}
        onClick={() =>
