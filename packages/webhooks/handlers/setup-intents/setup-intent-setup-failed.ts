...
export const handleSetupIntentSetupFailed = async (event: Stripe.Event) => {
  try {
    ...
    // Update the setup intent record with the failure details
    await db
      .update(setupIntents)
      .set({
    ...
    console.error('Error handling setup.intent.setup_failed event:', error)
    throw error
  }
}
